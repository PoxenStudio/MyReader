...
MyBooks is a cross-platform ebook reader forked from Readest which was built as a **Next.js 16 + Tauri v2** hybrid app. It's part of a pnpm monorepo at `/app/`. The app runs on web (CloudFlare Workers), desktop (macOS/Windows/Linux via Tauri), and mobile (iOS/Android via Tauri).
