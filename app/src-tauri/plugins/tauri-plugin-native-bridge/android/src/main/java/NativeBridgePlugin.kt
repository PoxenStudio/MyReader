...
class NativeBridgePlugin(private val activity: Activity): Plugin(activity) {
    ...
    // Tauri's Android `Plugin` base class has no `onActivityResult` override
    // point (unlike `onNewIntent`, which is genuinely forwarded) — activity
    // results are routed through `startActivityForResult(invoke, intent,
    // callbackName)`, which registers `invoke` against `callbackName` and
    // replays it into the matching `@ActivityCallback` method once the
    // system picker returns. The previous implementation called the raw
    // `Activity.startActivityForResult` + a same-named `handleActivityResult`
    // that nothing ever invoked, so the picker promise on the JS side hung
    // forever after the user chose a folder.
    @Command
    fun select_directory(invoke: Invoke) {
        try {
            val intent = Intent(Intent.ACTION_OPEN_DOCUMENT_TREE)
            startActivityForResult(invoke, intent, "handleDirectoryPickerResult")
        } catch (e: Exception) {
            val result = JSObject()
    ...
    @ActivityCallback
    private fun handleDirectoryPickerResult(invoke: Invoke, result: ActivityResult) {
        handleDirectorySelected(result.data?.data, invoke)
    }
