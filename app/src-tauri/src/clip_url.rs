...
/// are English / MyReader's dark palette so a caller that omits a field
...
impl ClipOptions {
    fn window_title(&self) -> &str {
        self.window_title
            .as_deref()
            .unwrap_or("Saving to your MyReader library…")
    }
    fn overlay_title(&self) -> &str {
        self.overlay_title.as_deref().unwrap_or("Saving to MyReader")
    }
    ...
    fn saved_title(&self) -> &str {
        self.saved_title.as_deref().unwrap_or("Saved to MyReader")
    }
...
pub async fn clip_url(
    ...
    // chrome style MyReader's main/reader windows use so it doesn't read
