...
//
// - `env_disable`: MYBOOKS_DISABLE_UPDATER is set (explicit opt-out).
...
fn updater_disabled() -> bool {
    let env_disable = std::env::var("MYBOOKS_DISABLE_UPDATER").is_ok();
    #[cfg(target_os = "linux")]
    {
        let is_flatpak =
...
pub fn run() {
    ...
    // Proxy::system() by default. Browsers automatically bypass proxy for local
    // addresses, but reqwest does not. Without this, tauriFetch to self-hosted
    // servers on localhost or LAN returns 502 when a system proxy is configured.
    // Setting NO_PROXY before any reqwest client is built makes Proxy::system()
    // exclude these addresses. Only set if the user hasn't already configured it.
    ...
    let builder = builder.plugin(
        tauri_plugin_single_instance::Builder::new()
            ...
            .dbus_id("com.poxenstudio.mybooks".to_owned())
            .build(),
    );
    ...
    builder
        .setup(|#[allow(unused_variables)] app| {
            ...
            let init_script = format!(
                r#"
                    if ({is_eink}) window.__MYBOOKS_IS_EINK = true;
                    if ({cli_access}) window.__MYBOOKS_CLI_ACCESS = true;
                    if ({is_appimage}) window.__MYBOOKS_IS_APPIMAGE = true;
                    if ({updater_disabled}) window.__MYBOOKS_UPDATER_DISABLED = true;
...
mod tests {
    ...
    fn env_opt_out_disables_on_any_desktop() {
        // MYBOOKS_DISABLE_UPDATER is an explicit opt-out on every desktop OS.
