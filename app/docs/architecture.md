# MyReader Architecture

This document gives a system-level view of MyReader: how the pieces fit together,
...
MyReader is a single TypeScript/React codebase (`app`) compiled into
multiple targets:
...
- two **side surfaces**: a "Send to MyReader" browser extension
  (`app/extension/send-to-readest`) and a Windows thumbnail
  shell extension (`app/extensions/windows-thumbnail`)
...
flowchart LR
    subgraph Clients
        ...
        Ext["Browser extension<br/>(Send to MyReader)"]
        WinExt["Windows shell ext<br/>(thumbnail provider)"]
    end

    subgraph Backend["MyReader backend (Next.js routes + Cloudflare Worker)"]
...
`window.__MyReader_RUNTIME_CONFIG = {...}` as a JavaScript file. It is loaded as
...
flowchart TB
    ...
    Send["app/send<br/>(send-to-MyReader inbox)"]
...
forked copy of the Foliate JS engine. MyReader's reader code in `app/reader` and
...
send/inbox.ts            -> "Send to MyReader" inbox listing
...
`app/workers/send-email` is a separate Cloudflare Worker
(deployed independently from the main app) responsible for the "Send to MyReader
...
"Send to MyReader" article-to-EPUB conversion.

### 6.11 Send to MyReader

End-to-end pipeline:

1. The browser extension (`app/extension/send-to-readest`) or the
...
flowchart LR
    ...
    subgraph DeployTargets
        ...
        DDocker["Docker image<br/>(ghcr.io/poxenstudio/myreader)"]
