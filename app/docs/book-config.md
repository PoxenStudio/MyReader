...
`schemaVersion` identifies the raw `config.json` schema written by MyReader.
Current version:
...
overrides and are merged with defaults when MyReader loads the config.
...
For notes in `booknotes`, MyReader stores note ranges with:
