## MyReader Design Language

MyReader's UI is **Adwaita-aligned**, **e-ink-first**, **cross-platform-aware**. This doc is the
...
MyReader's visual language descends from **Adwaita / libadwaita** — GNOME's design system —
...
- **Cross-platform reality.** MyReader runs on macOS, Windows, Linux, iOS, Android, web. The
...
MyReader ships with RTL languages enabled. **Never use direction-bound Tailwind
...
<div className='w-full'>
  ...
  <p className='text-base-content/70 text-sm leading-relaxed'>
    {_('Connect MyReader to external services for sync, highlights, and catalogs.')}
  </p>
</div>
```
...
- **Card / View**: `rounded-lg` (8px) — MyReader's house radius. Adwaita uses 9px; 8px is
...
Delete, remove, irreversible. Adwaita uses `destructive-action`. MyReader uses red
...
A MyReader-named archetype for "add another row to the list above" affordances. The two
...
These names come from libadwaita and apply 1:1 to MyReader's lists. Use the names in code
...
`snapHeight` prop. Adwaita doesn't have a native sheet but MyReader's mobile pattern is
the closest analog.
...
MyReader ships on **macOS, Windows, Linux, iOS, Android, web**. Adwaita is desktop-GNOME-
...
- Material 3 conventions that conflict with Adwaita (FABs, elevation shadows, ripple
  inks): **don't** copy them. MyReader's identity is Adwaita; the user is reading on
...
- Native Adwaita territory. MyReader can match host theme for window chrome (Tauri
...
- Title bar height matches platform convention; internal layout follows MyReader's
  Adwaita palette.
...
Why: hard-coded colors don't theme. MyReader has 11 themes plus user-defined custom themes.
...
- **Adwaita / libadwaita**: GNOME's design system and widget toolkit. Source of MyReader's
  visual lineage.
- **AdwActionRow / AdwSwitchRow / AdwComboRow / AdwExpanderRow**: libadwaita's row
  primitives. MyReader mirrors these conceptually with custom React components.
...
- **ListExtension**: MyReader-named archetype for "+ add new row" buttons (§4.6).
...
### 14. Library data model notes

Not a visual rule — recorded here per maintainer request so storage semantics don't drift
out of sync with the UI that gates on them.

`Book.storageType` (`src/types/book.ts`) is `'local' | 'cloud'`:

- **`'local'`**: the book file lives only on this device. Never uploaded, or its cloud
  copy was deleted.
- **`'cloud'`**: the book is backed by the MyBooks server. This includes books fetched
  from the MyBooks catalog (hash prefixed `cloud-`, see `isCloudBook` in
  `src/utils/bookConverter.ts`) **and** local books the user has uploaded — once
  `cloudService.uploadBook` succeeds, it flips `storageType` to `'cloud'` and stamps
  `uploadedAt`/`downloadedAt`/`coverDownloadedAt`, so an uploaded book is treated exactly
  like one already downloaded from the cloud (no redundant re-download prompt).

UI gating that depends on this (`BookshelfItem.tsx` context menu):

- "Upload Book" shows only for `storageType === 'local' && !uploadedAt && !downloadedAt`.
- "Download Book" / "Download in Format" shows only for `storageType === 'cloud' && !downloadedAt`.

When adding new cloud-aware affordances, gate on `storageType` (not just `uploadedAt`/
`downloadedAt` alone) so local-only and MyBooks-backed books stay visually and
behaviorally distinct.

---

### 15. Login state / connection state

MyReader works fully offline as a local reader. Login and MyBooks connectivity are two
independent, orthogonal states — never conflate them, and never gate local-bookshelf
access on either.

#### Connection state (`useMyBooksConnectionStatus`, `src/store/mybooksStatusStore.ts`)

Whether the configured MyBooks server is reachable, independent of whether the user is
logged in:

- **`'unconfigured'`**: no `mybooks_host` has ever been set.
- **`'unreachable'`**: a host is configured but the last request failed to reach it.
- **`'connected'`**: a host is configured and the last request succeeded (optimistic
  default — there's no separate "checking" state).

#### Login state (`AuthContext.status`, `src/context/AuthContext.tsx`)

- **`'logged_out'`**: no account, no guest session.
- **`'guest'`**: connected to a MyBooks host without real credentials, via "Continue as
  Guest" in `LoginDialog`. A guest gets a sentinel `user` object so it's treated exactly
  like `'logged_in'` everywhere that gates on `!!user` (cloud sync, OPDS, sharing,
  translation quota, etc.) — the **only** place that needs to special-case `isGuest` is
  avatar/nickname display (no real profile to show).
- **`'logged_in'`**: a real MyBooks/Supabase session.

#### UI rules

- **Disable, don't hide.** Cloud-only nav items (LibraryDrawer's account/reading/browse
  sections, SettingsMenu's account-gated items) stay visible but rendered
  non-interactive (`aria-disabled`, dimmed) when unavailable, so users can see what
  signing in unlocks. The local bookshelf (`home_links` in `LibraryDrawer`) is always
  enabled.
- **Cloud bookshelf availability** = `status !== 'logged_out' && connectionStatus ===
  'connected'`. Both conditions are required — being logged in against an unreachable
  server is still "local reader only", same as a connected server with no account.
- **LibraryHeader shows connection and login as two separate indicators**, never merged
  into one icon: a cloud icon (or nothing, when `'connected'`) for connection state, and
  a login button / avatar for login state. The avatar lives here (not in LibraryDrawer)
  so it's visible regardless of which drawer/panel is open.
- **Default icons**: logged out → `MdPersonOutline`; guest → `MdOutlineNoAccounts`
  (closest available Material Symbol to "this isn't a real account"; react-icons/md
  doesn't ship the MDI `account-alert-outline` glyph); logged in with no avatar →
  the existing `MdPerson`/`PiUserCircleCheck` fallback.
- **No forced login on launch.** MyReader never redirects to a login screen on open —
  login is always a `LoginDialog` the user opens on demand (`useAuthUIStore`), never a
  route navigation. `navigateToLogin()` opens the dialog; it does not change routes.
