...
locked a permanent strategic posture: **MyReader stays private-only**.
...
- Local on-disk paths are per-adapter. Existing `BaseDir`s
  ...
  kinds.
- Cloud key for binary files: `${userId}/MyReader/replicas/<kind>/<id>/<filename>`.
...
- User sets a **sync passphrase** (separate from MyReader auth password)
...
| `UnsupportedAlgError`      | yes     | skip + log + suggest update                 | "Update MyReader to read this data" |
...
| `SchemaTooNewError`        | yes     | skip + log + offer update                   | "Update MyReader"                   |
