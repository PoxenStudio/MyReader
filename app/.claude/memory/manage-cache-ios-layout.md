...
On-device analysis (dev build `com.poxenstudio.myreader`, pulled via `xcrun devicectl device copy from --domain-type appDataContainer`):

- The `'Cache'` base = Tauri `appCacheDir()` = **`Library/Caches/com.poxenstudio.myreader`** only — NOT all of `Library/Caches`. On the test device this held ~272 MB: a 249 MB duplicate dictionary (`concise-enhanced.mdx`, canonical copy lives in `App Support/MyReader/Dictionaries/<id>/`), ~23 MB duplicate import-staged epubs (canonical in `App Support/MyReader/Books/<hash>/`), the `search/` results cache, plus tiny system scratch. All safe to clear — every large item is an orphaned import/download staging duplicate.
...
- Never clear `Library/Application Support/com.poxenstudio.myreader/MyReader` — the real Books/Dictionaries/Fonts/DB (~1.7 GB).
