...
description: MyReader has a design system doc — codify recurring UI/UX rules there, don't just apply them ad-hoc. Memory points at the canonical location and the patterns it covers.
type: feedback
...
**Why:** MyReader's UI is Adwaita-aligned, e-ink-first, cross-platform-aware.
...
- §2.8: RTL — always use logical properties (`ps`/`pe`/`ms`/`me`/`text-start`
  ...
  this — MyReader ships RTL languages.
...
- `app/src/components/settings/SubPageHeader.tsx` — title +
  description sub-page primitive that embodies §2.9.
- `app/src/components/settings/integrations/` — the canonical
  reference implementation of the boxed-list-with-rows pattern.
- `app/src/styles/globals.css` — eink overlay rules at `[data-eink='true']`.
