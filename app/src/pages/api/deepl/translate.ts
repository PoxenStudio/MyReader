...
import { getDailyTranslationPlanData, validateUserAndToken } from '@/utils/access';
...
const checkDailyUsage = async (userId: string, chars: number) => {
  const { quota: dailyQuota } = getDailyTranslationPlanData();
...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  const { DEEPL_PRO_API, DEEPL_FREE_API, DEEPL_PRO_API_KEYS } = process.env;
  const deepFreeApiUrl = DEEPL_FREE_API || DEFAULT_DEEPL_FREE_API;
  const deeplProApiUrl = DEEPL_PRO_API || DEFAULT_DEEPL_PRO_API;

  // DeepL's own free/pro API tiers are a server-side key configuration, not
  // tied to any per-user plan — use the pro endpoint whenever pro keys are set.
  const deeplApiUrl = DEEPL_PRO_API_KEYS ? deeplProApiUrl : deepFreeApiUrl;
  const deeplAuthKey =
  ...
  try {
    const translations = await Promise.all(
      text.map(async (singleText) => {
        ...
        await checkDailyUsage(user?.id, singleText.length);
