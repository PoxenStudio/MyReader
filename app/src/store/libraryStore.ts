...
// Pushes a local readingStatus change back to the MyBooks server for books
// downloaded from MyBooks. Fire-and-forget: sync failures shouldn't block
// the local update.
...
export const useLibraryStore = create<LibraryState>((set, get) => ({
  ...
  updateBookProgress: (hash, progress, readingStatus) => {
    ...
    syncReadingStatusToMyBooks(updatedBook, book.readingStatus);
  },
  ...
  updateBook: async (envConfig: EnvConfigType, book: Book) => {
    ...
    const oldBook = idx !== undefined ? library[idx] : undefined;
    // Build the new library immutably — never mutate the previous-state array.
    const newLibrary =
      idx !== undefined
        ? [...library.slice(0, idx), book, ...library.slice(idx + 1)]
        : [...library, book]; // Add the book if it doesn't exist in the library
    console.log('[libraryStore] updateBook. new library size:', newLibrary.length);
    set({
      library: newLibrary,
      hashIndex: buildHashIndex(newLibrary),
      visibleLibrary: newLibrary.filter((b) => !b.deletedAt),
    });
    syncReadingStatusToMyBooks(book, oldBook?.readingStatus);
  ...
  updateBooks: async (
    ...
    const oldByHash = new Map(library.map((b) => [b.hash, b]));
    ...
    for (const book of books) {
      syncReadingStatusToMyBooks(book, oldByHash.get(book.hash)?.readingStatus);
    }
