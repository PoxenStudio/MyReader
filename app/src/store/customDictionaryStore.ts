...
interface DictionaryStoreState {
  ...
  /**
   * Clears the `unavailable` flag on the dict matching `contentId`. No-op
   * if the dict isn't found.
   */
...
export const useCustomDictionaryStore = create<DictionaryStoreState>((set, get) => ({
  ...
  loadCustomDictionaries: async (envConfig) => {
    try {
      ...
      // Drop providerOrder / providerEnabled entries whose customDictionaries row is tombstoned.
