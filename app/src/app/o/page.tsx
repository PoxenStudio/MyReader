...
import { DOWNLOAD_MYBOOKS_URL, MYBOOKS_WEB_BASE_URL } from '@/services/constants';
...
const ANDROID_PACKAGE = 'com.poxenstudio.mybooks';
...
const OpenAnnotationLanding = () => {
  ...
  useEffect(() => {
    ...
    if (platform === 'android-chromium') {
      const absoluteFallback = `${MYBOOKS_WEB_BASE_URL}${webReaderUrl}`;
  ...
  return (
    <main className='bg-base-200 flex min-h-dvh flex-col items-center justify-center p-4 sm:p-8'>
      <Card>
        <BrandHeader
          title={_('Open in MyReader')}
          subtitle={
            showManualOpen
              ? _("If MyReader didn't open automatically, choose an option below:")
        ...
        {!showManualOpen && (
          ...
          >
            ...
            <span className='text-base-content/70 text-sm'>{_('Opening MyReader...')}</span>
          </div>
        )}
        ...
        >
          ...
          <p className='text-base-content/60 mt-3 text-center text-xs'>
            {_("Don't have MyReader?")}{' '}
            <a
              href={DOWNLOAD_MYBOOKS_URL}
              target='_blank'
              rel='noopener'
