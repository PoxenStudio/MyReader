...
const TTSPlayerSheet = ({
  ...
  // Offline audio (pre-downloading Read Aloud audio per chapter) requires a
  // signed-in account; signed-out users are routed to sign in first.
  const isDownloadPremium = !!user;
  ...
  // Signed-in users drill into the per-chapter download view; signed-out
  // users are routed to sign in, the sheet closing first so the navigation
