...
import { getCoverFilename, getBookHash, getLocalBookFilename } from '@/utils/book';
 ...
 *
 * A single Reader-level hook drives both progress and booknote sync against
 * `<rootPath>/MyReader/books/<hash>/config.json`. Pull-once on book open,
...
export const useWebDAVSync = (bookKey: string) => {
      ...
      if (wantNotes && result.mergedNotes) {
        ...
        for (const note of result.mergedNotes) {
          ...
          if (note.deletedAt && (!prev || !prev.deletedAt)) {
            ...
            getViewsById(getBookHash(bookKey)).forEach((v) => removeBookNoteOverlays(v, note));
  ...
  // useEffect below doesn't have to re-bind on every render.
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!merged) {
        ...
        // structure (MyReader/books/<hash>/) by uploading the local config.
