...
const LibraryPageContent = ({ searchParams }: { searchParams: ReadonlyURLSearchParams | null }) => {
  ...
  // panel sees imported fonts even when opened straight from the library.
  useCustomFonts();
  ...
  const [selectedMetaType, setSelectedMetaType] = useState<
    'author' | 'tag' | 'publisher' | 'series' | 'language' | 'rating' | 'categories'
  >('categories');
    ...
    async (appService: AppService, openWithFiles: string[], libraryBooks: Book[]) => {
      ...
      for (const file of openWithFiles) {
        ...
        try {
          ...
          // A file shared into MyReader on mobile (the OS share-sheet) is a
          // "Send to MyReader" capture — force it to the cloud so it syncs to
  ...
  // Load cloud books when source/type changes
  useEffect(() => {
    const source = searchParams?.get('source') || 'local';
    ...
    // Update meta type when type changes
    const metaTypes: Array<
      'author' | 'tag' | 'publisher' | 'series' | 'language' | 'rating' | 'categories'
    > = ['author', 'tag', 'publisher', 'series', 'language', 'rating', 'categories'];
    if (metaTypes.includes(type as any)) {
      setSelectedMetaType(type as any);
      setShowAllMeta(false);
    }

    // Load cloud books - only when itemName is specified (viewing specific category/author/etc)
    // or when type is not a meta type
    ...
    // Load books only when viewing a specific item or when type is not a meta type
    if (itemName || !metaTypes.includes(type as any)) {
      loadCloudBooks();
    } else {
      // Reset books when viewing meta list
      setCloudBooks([]);
      setCloudBooksTotal(0);
      setCloudBooksPage(1);
      setCloudBooksLoading(false);
  ...
  const importBooks = async (files: SelectedFile[], groupId?: string) => {
    ...
    // MyReader's own Books/ prefix is resolved once at app init and persisted
  ...
  const handleBookDownload = useCallback(
    async (book: Book, downloadOptions: { redownload?: boolean; queued?: boolean } = {}) => {
      console.log('handlebookdownload', book);
      const { redownload = false, queued = false } = downloadOptions;
      if (redownload || !queued) {
        console.log('handlebookdownload #1');
        try {
          console.log(
            '[handleBookDownload] calling downloadBook for:',
            book.title,
            'hash:',
            book.hash,
          );
          ...
          console.log('[handleBookDownload] calling updateBook');
          await updateBook(envConfig, book);
          console.log('[handleBookDownload] updateBook call completed');
          eventDispatcher.dispatch('toast', {
            type: 'info',
            timeout: 2000,
  ...
  const handleBookDelete = (deleteAction: Exclude<DeleteAction, 'remote'>) => {
    return async (book: Book) => {
      ...
      try {
        ...
        // Cloud deletion is just local bookkeeping (clearing uploadedAt) —
        // mybooks has no remote-delete API reachable by regular users, see
        // cloudService.deleteBook — so it doesn't need the transfer queue.
  ...
  // Deleting a book while browsing the cloud shelf removes it from the
  // shared MyBooks cloud library (admin-only) rather than touching local files,
  // so it updates `cloudBooks` directly instead of the local library store.
  const handleCloudDelete = async (book: Book) => {
    try {
      await appService?.deleteBook(book, 'remote');
      setCloudBooks((prev) => prev.filter((b) => b.hash !== book.hash));
      setCloudBooksTotal((prev) => Math.max(0, prev - 1));
      eventDispatcher.dispatch('toast', {
        type: 'info',
        timeout: 1000,
  ...
  const validatePickedDirectory = (path: string): boolean => {
    ...
    if (isOnMyIPhoneRoot) {
      eventDispatcher.dispatch('toast', {
        ...
        timeout: 6000,
        message: _(
          'iOS doesn\'t allow importing the "On My iPhone" root. Open it and pick a specific subfolder (e.g. MyReader, Downloads), then try again.',
        ),
      });
      return false;
    }
      ...
      <div className='flex flex-col flex-1 min-w-0 h-full relative'>
        ...
        {showDetailsBook && (
          <BookDetailModal
            ...
            handleBookUpload={handleBookUpload}
            handleBookDownload={handleBookDownload}
            handleBookDelete={source === 'cloud' ? handleCloudDelete : handleBookDelete('local')}
            deleteDisabled={source === 'cloud' && !isAdmin}
            deleteConfirmMessage={
              source === 'cloud'
                ? _('Are you sure to delete this book from MyBooks?')
