...
export const useOpenBook = ({ setLoading, handleBookDownload }: UseOpenBookOptions) => {
  ...
  const makeBookAvailable = useCallback(
    async (book: Book) => {
      ...
      // handles the case where such a book's local file is gone. Cloud-library
      // books (metadata-only rows browsed from the cloud shelf) never carry
      // `uploadedAt` from this device, so they need the explicit isCloudBook
      // check to still go through the on-demand download path below.
      if (!isCloudBook(book) && !book.uploadedAt) return true;
