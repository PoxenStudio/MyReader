...
const Bookshelf: React.FC<BookshelfProps> = ({
  ...
  const [sendToDeviceBook, setSendToDeviceBook] = useState<Book | null>(null);

  useEffect(() => {
    const handleSendToDeviceIntent = (event: CustomEvent) => {
      const book = (event.detail as { book?: Book } | undefined)?.book;
      if (!book) return;
      if (!user) {
        eventDispatcher.dispatch('toast', {
          type: 'info',
          message: _('Sign in to send books to a device'),
          timeout: 2500,
        });
        return;
      }
      setSendToDeviceBook(book);
    };
    eventDispatcher.on('show-send-to-device-dialog', handleSendToDeviceIntent);
    return () => {
      eventDispatcher.off('show-send-to-device-dialog', handleSendToDeviceIntent);
    };
  }, [user, _]);
