...
export interface ImportFromFolderResult {
   ...
   * — MyReader will read each file straight from its original location
   * instead of copying it into Books/<hash>/. Sidecars (cover, config,
   * notes) still live in MyReader's data dir. Deleting such a book from
   * MyReader only removes those MyReader-side sidecars; the original
   * source file under the registered folder is left untouched.
   * Defaults to `false`, which keeps the legacy "copy into MyReader"
            ...
            <span className='select-none'>
              ...
              <span className='text-base-content/60 block text-xs'>
                {readInPlaceLocked
                  ...
                  : _(
                      'Read books from their original folders instead of copying them into the library. Saves disk space.',
                    )}
              </span>
            </span>
          </label>
