...
import { Book, BookFormat, BooksGroup, ReadingStatus } from '@/types/book';
import {
  buildCloudBookHash,
  getCloudBookId,
...
const BookshelfItem: React.FC<BookshelfItemProps> = ({
  ...
  const { updateBook, getBookByHash } = useLibraryStore();
  ...
  const makeBookAvailable = async (book: Book) => {
    if (isCloudBook(book) || (book.uploadedAt && !book.downloadedAt)) {
  ...
  const handleReadInFormat = useCallback(
    async (book: Book, format: BookFormat) => {
      const id = getCloudBookId(book.hash);
      if (id === null) return;
      const targetBook =
        getBookByHash(buildCloudBookHash(id, format)) ?? getFormatVariantBook(book, format);
      if (!targetBook) return;
      const available = await makeBookAvailable(targetBook);
      if (!available) return;
      if (appService?.hasWindow && settings.openBookInNewWindow) {
        showReaderWindow(appService, [targetBook.hash]);
      } else {
        setTimeout(() => {
          navigateToReader(router, [targetBook.hash]);
        }, 0);
      }
    },
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const handleDownloadInFormat = useCallback(
    async (book: Book, format: BookFormat) => {
      const id = getCloudBookId(book.hash);
      if (id === null) return;
      const targetBook =
  ...
  const bookContextMenuHandler = async (book: Book) => {
    ...
    const sendToDeviceMenuItem = await MenuItem.new({
      text: _('Send to Device'),
      action: async () => {
    ...
    const readInFormatMenuItem =
      isCloudBook(book) && availableFormats.length > 1
        ? await Submenu.new({
            text: _('Read in Format'),
            items: await Promise.all(
              availableFormats.map((format) =>
                MenuItem.new({
                  text: format,
                  action: async () => {
                    handleReadInFormat(book, format);
                  },
                }),
              ),
            ),
        ...
        : null;
    const downloadInFormatMenuItem =
      isCloudBook(book) && availableFormats.length > 1
        ? await Submenu.new({
            text: _('Download in Format'),
            items: await Promise.all(
              availableFormats.map((format) =>
                MenuItem.new({
                  text: format,
                  action: async () => {
                    handleDownloadInFormat(book, format);
                  },
                }),
              ),
            ),
    ...
    if (!isCloudLibrary && (book.readingStatus === 'finished' || book.readingStatus === 'unread')) {
      menu.append(clearStatusMenuItem);
    }
    menu.append(showBookDetailsMenuItem);
    if (isCloudLibrary && readInFormatMenuItem) {
      menu.append(readInFormatMenuItem);
    }
    if (!isCloudLibrary) {
