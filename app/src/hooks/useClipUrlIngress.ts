...
/**
 * Handle "Share to MyReader" article URLs from the OS share sheet
 ...
 *      NSUserDefaults at `group.com.poxenstudio.mybooks`, and the host
 ...
 *   - http(s) (not file://, content://, mybooks://, blob:, data:)
...
export function useClipUrlIngress() {
  ...
  useEffect(() => {
    ...
    const handle = (url: string) => {
      ...
      //       mybooks://clip?url=<encoded>
      const isClipUrl =
        url.startsWith('mybooks://clip?') ||
        url.startsWith('mybooks://clip/') ||
