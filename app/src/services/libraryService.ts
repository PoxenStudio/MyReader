...
export async function loadLibraryBooks(
  ...
  console.log(`[Library Service] library file ${libraryFilename}`);
  ...
  console.log(`[Library Service] loaded ${books.length} books from ${libraryFilename}`);

  return books;
}

export async function saveLibraryBooks(fs: FileSystem, books: Book[]): Promise<void> {
  console.log('[libraryService] saveLibraryBooks. Saving books count:', books.length);
