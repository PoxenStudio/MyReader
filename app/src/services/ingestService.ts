...
export interface IngestFileDeps {
  ...
  isLoggedIn: boolean;
  /**
   * Pre-resolved absolute path to MyReader's own `Books/` directory. When
 ...
 *     explicitly told MyReader to read in place. The MyReader data location
 *     (`customRootDir`) is intentionally NOT an in-place trigger; that
 *     directory is MyReader's own home and may freely contain hash copies.
 *   - The path is NOT inside MyReader's own managed books directory
 *     (`appBooksPrefix`, e.g. `<AppData>/Books/`). Anything in that subtree
 *     is a hash copy under MyReader's control, no point marking it in-place.
...
function shouldImportInPlace(
  ...
  // If the file already lives inside MyReader's own managed books directory
...
export async function ingestFile(
  ...
  // and shouldn't pollute the user's cloud library.
