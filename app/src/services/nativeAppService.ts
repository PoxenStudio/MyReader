...
declare global {
  interface Window {
    __MYBOOKS_IS_EINK?: boolean;
    __MYBOOKS_IS_APPIMAGE?: boolean;
    __MYBOOKS_UPDATER_DISABLED?: boolean;
  }
}
...
export class NativeAppService extends BaseAppService {
  ...
  override isAppImage = Boolean(window.__MYBOOKS_IS_APPIMAGE);
  override isEink = Boolean(window.__MYBOOKS_IS_EINK);
  ...
  async migrate20251029() {
    ...
    const oldDir = await join(rootPath, 'Images', 'MyReader', 'Images');
    const dirToDelete = await join(rootPath, 'Images', 'MyReader');
    ...
    if (!(await this.fs.exists(oldDir, 'None'))) {
      console.log('Migration 20251029: legacy Images/MyReader/Images not found, skipping.');
      return;
    }
