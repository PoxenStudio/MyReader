...
export type WebDAVSyncStrategy = 'prompt' | 'silent' | 'send' | 'receive';
...
export interface WebDAVSettings {
  enabled: boolean;
  // Which credentials this connection uses. 'mybooks' reuses the
  // currently signed-in MyReader account (server/username/password are
  // resolved at use-time, not persisted here). Defaults to 'custom' for
  // settings written before this field existed.
...
export interface SystemSettings {
   ...
   * Calibre, Moon+ Reader) that MyReader should read in place instead of
