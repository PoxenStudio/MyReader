...
export const resolveNightlyUpdate = async (
  ...
  const [nightly, stable] = await Promise.all([
    fetchManifest(fetchFn, MYBOOKS_NIGHTLY_UPDATER_FILE),
    fetchManifest(fetchFn, MYBOOKS_UPDATER_FILE),
  ]);
  const sources: Array<[UpdateManifest | null, string]> = [
    [nightly, MYBOOKS_NIGHTLY_UPDATER_FILE],
    [stable, MYBOOKS_UPDATER_FILE],
  ];
  ...
  try {
    if (updateChannel === 'nightly') {
      const platformKey = getNightlyPlatformKey(
        ...
        Boolean((window as { __MYBOOKS_IS_APPIMAGE?: boolean }).__MYBOOKS_IS_APPIMAGE),
      );
    ...
    if (['macos', 'windows', 'linux'].includes(OS_TYPE)) {
      ...
      try {
        const response = await fetch(MYBOOKS_UPDATER_FILE, { connectTimeout: 5000 });
...
export const checkAppReleaseNotes = async (isAutoCheck = true) => {
  ...
  if ((lastShownVersion && semver.gt(currentVersion, lastShownVersion)) || !isAutoCheck) {
    try {
      ...
      const res = await fetchFunc(MYBOOKS_CHANGELOG_FILE);
      if (res.ok) {
