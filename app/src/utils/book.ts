...
/**
 * Extracts the book hash from a book key or returns it unchanged if it is already a pure hash.
 *
 * Book keys have the format "<hash>-<7-char-uniqueId>" (e.g. "cloud-123-abc1234").
 * Book hashes may themselves contain dashes (e.g. cloud books use "cloud-<id>").
 * Using `split('-')[0]` would incorrectly return "cloud" for "cloud-123-abc1234".
 *
 * The uniqueId produced by `uniqueId()` is always exactly 7 base-36 chars ([0-9a-z]).
 * If the string ends with "-<7-base36-chars>" we treat that suffix as a uniqueId and strip it.
 */
export function getBookHash(bookKey: string): string {
  const lastDash = bookKey.lastIndexOf('-');
  if (lastDash !== -1) {
    const suffix = bookKey.substring(lastDash + 1);
    if (suffix.length === 7 && /^[0-9a-z]+$/.test(suffix)) {
      return bookKey.substring(0, lastDash);
    }
  }
  return bookKey;
}
