import { MYBOOKS_WEB_BASE_URL, SHARE_BASE_URL, SHARE_TOKEN_LENGTH } from '@/services/constants';
...
export const parseShareDeepLink = (url: string): ShareDeepLink | null => {
  ...
  if (parsed.protocol === 'readest:') {
    // For mybooks://share/{token} the host portion holds the path segment
  ...
  if (parsed.protocol === 'https:' || parsed.protocol === 'http:') {
    if (!isWebMyBooksHost(parsed.host)) return null;
...
const isWebMyBooksHost = (host: string): boolean => {
  // Matches the production host and any preview domain MyReader may serve from.
  ...
  if (host === new URL(MYBOOKS_WEB_BASE_URL).host) return true;
