...
export class TxtToEpubConverter {
  ...
  private async convertSmallFile(options: Txt2EpubOptions): Promise<ConversionResult> {
    ...
    // zip.js's BlobWriter can produce a Blob from a different realm than the
    // caller's global Blob/File (e.g. under jsdom), which fails an internal
    // instanceof-style check in File's constructor and silently stringifies
    // to "[object Blob]" instead of the zip bytes. Route through arrayBuffer()
    // first so File always gets a plain BufferSource.
    return {
      file: new File([await blob.arrayBuffer()], fileName),
      bookTitle,
