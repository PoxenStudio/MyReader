import { MYBOOKS_WEB_BASE_URL } from '@/services/constants';
 ...
 * `mybooks://` app deeplink or the universal `https://` web link.
 */
...
export const buildAnnotationWebUrl = ({ bookHash, noteId, cfi }: AnnotationDeepLink): string => {
  const base = `${MYBOOKS_WEB_BASE_URL}${ANNOTATION_PATH_PREFIX}${bookHash}/annotation/${noteId}`;
