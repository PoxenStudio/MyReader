...
import {
  ...
  CJK_FONTS_PATTENS,
  BOOK_IDS_SEPARATOR,
  DOWNLOAD_MYBOOKS_URL,
  MYBOOKS_WEB_BASE_URL,
  MYBOOKS_NODE_BASE_URL,
  MYBOOKS_UPDATER_FILE,
  MYBOOKS_CHANGELOG_FILE,
  MYBOOKS_PUBLIC_STORAGE_BASE_URL,
  MYBOOKS_OPDS_USER_AGENT,
...
describe('services/constants', () => {
  ...
  describe('DEFAULT_SYSTEM_SETTINGS', () => {
    ...
    it('has boolean flags', () => {
      ...
      expect(DEFAULT_SYSTEM_SETTINGS.autoSetReadState).toBe(false);
  ...
  describe('URL constants', () => {
    ...
    it('DOWNLOAD_MYBOOKS_URL is a valid URL', () => {
      expect(DOWNLOAD_MYBOOKS_URL).toMatch(/^https:\/\//);
    });

    it('MYBOOKS_WEB_BASE_URL is a valid URL', () => {
      expect(MYBOOKS_WEB_BASE_URL).toMatch(/^https:\/\//);
    });

    it('MYBOOKS_NODE_BASE_URL is a valid URL', () => {
      expect(MYBOOKS_NODE_BASE_URL).toMatch(/^https:\/\//);
    });

    it('MYBOOKS_UPDATER_FILE is a URL ending with .json', () => {
      expect(MYBOOKS_UPDATER_FILE).toMatch(/^https:\/\//);
      expect(MYBOOKS_UPDATER_FILE).toMatch(/\.json$/);
    });

    it('MYBOOKS_CHANGELOG_FILE is a URL ending with .json', () => {
      expect(MYBOOKS_CHANGELOG_FILE).toMatch(/^https:\/\//);
      expect(MYBOOKS_CHANGELOG_FILE).toMatch(/\.json$/);
    });

    it('MYBOOKS_PUBLIC_STORAGE_BASE_URL is a valid URL', () => {
      expect(MYBOOKS_PUBLIC_STORAGE_BASE_URL).toMatch(/^https:\/\//);
    });

    it('MYBOOKS_OPDS_USER_AGENT is a non-empty string', () => {
      expect(typeof MYBOOKS_OPDS_USER_AGENT).toBe('string');
      expect(MYBOOKS_OPDS_USER_AGENT.length).toBeGreaterThan(0);
    });
  });
