...
const settings: WebDAVSettings = {
  ...
  // below can hard-code "/MyReader/books/<hash>".
  rootPath: '/',
};
...
describe('deleteDirectory', () => {
  test('204 No Content resolves without throwing', async () => {
    ...
    await expect(deleteDirectory(config, '/MyReader/books/abc')).resolves.toBeUndefined();
  });
  ...
  test('404 Not Found is treated as success (already gone)', async () => {
    ...
    await expect(deleteDirectory(config, '/MyReader/books/missing')).resolves.toBeUndefined();
  });

  test('401 Unauthorized throws AUTH_FAILED', async () => {
    fetchMock.mockResolvedValueOnce(buildResponse(401));
    await expect(deleteDirectory(config, '/MyReader/books/abc')).rejects.toMatchObject({
      code: 'AUTH_FAILED',
      status: 401,
    });
  });
  ...
  test('sends DELETE with explicit Depth: infinity header', async () => {
    ...
    fetchMock.mockResolvedValueOnce(buildResponse(204));
    await deleteDirectory(config, '/MyReader/books/abc');
    expect(fetchMock).toHaveBeenCalledTimes(1);
    const [url, init] = fetchMock.mock.calls[0]!;
    expect(url).toBe('https://dav.example.com/MyReader/books/abc');
...
describe('deleteRemoteBookDir', () => {
  ...
  test('targets the correct per-hash directory under <rootPath>/MyReader/books', async () => {
    ...
    expect(url).toBe('https://dav.example.com/MyDav/MyReader/books/abc123');
  });
});
