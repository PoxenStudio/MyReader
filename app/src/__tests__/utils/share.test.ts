...
describe('parseShareDeepLink', () => {
  ...
  it('parses mybooks://share/{token}', () => {
    expect(parseShareDeepLink(`mybooks://share/${VALID_TOKEN}`)).toEqual({ token: VALID_TOKEN });
  });
  ...
  it('rejects tokens of the wrong length', () => {
    expect(parseShareDeepLink('mybooks://share/short')).toBeNull();
    expect(parseShareDeepLink(`mybooks://share/${VALID_TOKEN}extra`)).toBeNull();
  });
  ...
  it('rejects tokens with disallowed characters', () => {
    ...
    expect(parseShareDeepLink(`mybooks://share/${bad}`)).toBeNull();
  });
  ...
  it('rejects mybooks:// URLs whose host is not "share"', () => {
    expect(parseShareDeepLink(`mybooks://book/${VALID_TOKEN}`)).toBeNull();
    expect(parseShareDeepLink(`mybooks://annotation/${VALID_TOKEN}`)).toBeNull();
  });
