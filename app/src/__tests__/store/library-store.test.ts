...
describe('libraryStore', () => {
  ...
  describe('updateBookProgress', () => {
    ...
    test('syncs to MyBooks when a MyBooks book transitions to finished', () => {
    ...
    test('does not sync when readingStatus is unchanged', () => {
      const books = [makeBook({ hash: 'a', bookId: 42, readingStatus: 'finished' })];
      useLibraryStore.getState().setLibrary(books);
    ...
    test('falls back to parsing the cloud-<id> hash when bookId is missing', () => {
      // Books downloaded before `bookId` was tracked only have it encoded in the hash.
      const books = [makeBook({ hash: 'cloud-99-epub', readingStatus: undefined })];
      useLibraryStore.getState().setLibrary(books);

      useLibraryStore.getState().updateBookProgress('cloud-99-epub', [100, 100], 'finished');

      expect(updateReadState).toHaveBeenCalledWith(99, 2);
    });
  });
  ...
  describe('updateBooks', () => {
    ...
    test('syncs to MyBooks for each updated book whose readingStatus changed', async () => {
      useLibraryStore
        .getState()
        .setLibrary([
          makeBook({ hash: 'a', bookId: 1, readingStatus: undefined }),
          makeBook({ hash: 'b', bookId: 2, readingStatus: 'unread' }),
        ]);
      const envConfig = makeEnvConfig({ saveLibraryBooks: vi.fn().mockResolvedValue(undefined) });

      await useLibraryStore
        .getState()
        .updateBooks(envConfig, [
          makeBook({ hash: 'a', bookId: 1, readingStatus: 'finished' }),
          makeBook({ hash: 'b', bookId: 2, readingStatus: 'unread' }),
        ]);

      expect(updateReadState).toHaveBeenCalledTimes(1);
      expect(updateReadState).toHaveBeenCalledWith(1, 2);
    });
  });

  describe('updateBook', () => {
    test('syncs to MyBooks when readingStatus changes for a MyBooks book', async () => {
      useLibraryStore
        .getState()
        .setLibrary([makeBook({ hash: 'a', bookId: 7, readingStatus: 'unread' })]);
      const envConfig = makeEnvConfig({ saveLibraryBooks: vi.fn().mockResolvedValue(undefined) });

      await useLibraryStore
        .getState()
        .updateBook(envConfig, makeBook({ hash: 'a', bookId: 7, readingStatus: undefined }));

      expect(updateReadState).toHaveBeenCalledWith(7, 1);
    });

    test('does not sync when the book has no bookId', async () => {
      useLibraryStore.getState().setLibrary([makeBook({ hash: 'a', readingStatus: 'unread' })]);
      const envConfig = makeEnvConfig({ saveLibraryBooks: vi.fn().mockResolvedValue(undefined) });

      await useLibraryStore
        .getState()
        .updateBook(envConfig, makeBook({ hash: 'a', readingStatus: undefined }));
