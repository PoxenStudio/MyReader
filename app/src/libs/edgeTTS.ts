...
// `_(voice.name)` in TTSPlayerSheet, but the names themselves are computed at
// runtime from EDGE_TTS_VOICES ids, so the i18next scanner can't discover
// them by walking genVoiceList(). Every literal name is declared here so
// `pnpm i18n:extract` emits a translatable key for each one (see
// docs/i18n.md). Keep this list in sync with EDGE_TTS_VOICES — the
// `EDGE_TTS_VOICE_NAME_KEYS` unit test fails if a name goes unregistered.
export const EDGE_TTS_VOICE_NAME_KEYS = [
  _('Aarohi'),
  _('Abdullah'),
  _('Abeo'),
  _('Adri'),
