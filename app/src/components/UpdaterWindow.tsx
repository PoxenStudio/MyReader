...
export const UpdaterContent = ({
  ...
  useEffect(() => {
    ...
    const checkAndroidUpdate = async () => {
      ...
      const response = await fetch(MYBOOKS_UPDATER_FILE);
      ...
      if (semver.gt(data.version, currentVersion)) {
        ...
        const apkFilePath = await appService.resolveFilePath(
          `MyBooks_${data.version}_${arch}.apk`,
          'Cache',
        );
        setUpdate({
          currentVersion,
    ...
    const checkWindowsPortableUpdate = async () => {
      ...
      if (semver.gt(data.version, currentVersion)) {
        ...
        const exeFileName = `MyBooks_${data.version}_${arch}-portable.exe`;
    ...
    const checkAppImageUpdate = async () => {
      ...
      if (semver.gt(data.version, currentVersion)) {
        ...
        const downloadUrl = data.platforms[platformKey]?.url as string;
        const appImageFileName = `MyBooks_${data.version}_${arch}.AppImage`;
    ...
    const buildNightlyUpdate = (n: ResolvedNightlyUpdate): GenericUpdate => ({
      ...
      downloadAndInstall: async (onEvent) => {
        ...
        const ok = await verifyUpdateSignature(filePath, n.signature, MYBOOKS_UPDATER_PUBKEY);
        if (!ok) {
  ...
  useEffect(() => {
    const fetchChangelogs = async (fromVersion: string): Promise<Changelog[]> => {
      try {
        const fetch = isTauriAppPlatform() ? tauriFetch : window.fetch;
        const res = await fetch(MYBOOKS_CHANGELOG_FILE);
          ...
          {checkUpdate ? (
            <div className='text-base-content flex-grow text-sm'>
              <h2 className='mb-4 text-center font-bold sm:text-start'>
                {_('A new version of MyReader is available!')}
              </h2>
              <p className='mb-2'>
                {_('MyReader {{newVersion}} is available (installed version {{currentVersion}}).', {
                  newVersion,
                  currentVersion,
                })}
              </p>
                ...
                {changelogs.length > 0 && semver.gt(changelogs[0]!.version, currentVersion) ? (
                  <div className='flex gap-2'>
                    ...
                    {appService?.isAndroidApp && (
                      <Link
                        href='https://play.google.com/store/apps/details?id=com.poxenstudio.mybooks'
                        target='_blank'
                        rel='noopener noreferrer'
                        className='btn btn-primary btn-sm'
                      >
...
export const UpdaterWindow = () => {
  ...
  return (
    <Dialog
      id='updater_window'
      isOpen={isOpen}
      title={checkUpdate ? _('Software Update') : _("What's New in MyReader")}
