...
const WebDAVForm: React.FC<WebDAVFormProps> = ({ onBack }) => {
  ...
  // pre-filled. The 'mybooks' type never persists a serverUrl, so it's
  // considered configured as long as it's enabled.
  const isConfigured = !!stored?.enabled && (stored?.type === 'mybooks' || !!stored?.serverUrl);

  // For 'mybooks' connections, the actual server/username/password/root
  // aren't persisted — resolve them from the current MyReader login so the
  // browse pane and Sync now have something to talk to.
  const effective = useMemo(() => (stored ? resolveWebDAVSettings(stored) : stored), [stored]);

  // Resolved once — not reactive to login changes while this form is open.
  const myBooksCreds = useMemo(() => getMyBooksWebDAVCredentials(), []);

  // Which storage backend the (unconfigured) form is set up for.
  // Default to 'mybooks' when the user is already signed in to MyReader.
  const [connectType, setConnectType] = useState<WebDAVConnectionType>(
    stored?.type ?? (myBooksCreds ? 'mybooks' : 'custom'),
  );
  ...
  const handleTestConnection = useCallback(async () => {
    ...
    if (result.success) {
      ...
      setConnectionError(formatConnectError(_, result));
    }
  }, [_]);

  useEffect(() => {
    if (!isConfigured && connectType === 'mybooks' && myBooksCreds) {
      handleTestConnection();
    }
    // Run once on mount / when type switches to mybooks
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [connectType]);

  const handleConnect = async () => {
    if (connectType === 'mybooks') {
      const creds = getMyBooksWebDAVCredentials();
      if (!creds) {
        eventDispatcher.dispatch('toast', {
          type: 'error',
          message: _('Sign in to MyReader first to use it as your sync storage.'),
        });
        return;
      }
      setIsConnecting(true);
  ...
  const handleSyncNow = async () => {
    ...
    if (!stored?.enabled || !effective?.serverUrl) return;
    ...
    try {
      const result = await syncLibrary(effective, eligibleBooks, {
        ...
        downloadBookFile: isTauriAppPlatform()
          ? async (book, remotePath) => {
              ...
              const url = buildRequestUrl(effective.serverUrl, remotePath);
              const headers = {
                Authorization: buildBasicAuthHeader(effective.username, effective.password),
              };
  ...
  const description: string = isConfigured
    ? _('Browsing {{path}} on {{server}}', {
        path: normalizeRootPath(effective.rootPath || '/'),
        server: effective.serverUrl,
      })
  ...
  return (
    <div className='w-full'>
      ...
      {isConfigured ? (
        ...
        <div className='space-y-5'>
          <BoxedList>
            <SettingsRow label={_('Storage')}>
              <SettingsSelect
                value={connectType}
                onChange={(e) => setConnectType(e.target.value as WebDAVConnectionType)}
                ariaLabel={_('Storage')}
                options={[
                  { value: 'mybooks', label: _('MyReader') },
          ...
          >
            {connectType === 'mybooks' ? (
              <>
                <p className='text-base-content/70 px-1 text-sm'>
                  {myBooksCreds
                    ? _(
                        'Use your MyReader account ({{username}}) to sync to {{path}} on {{server}}.',
                        {
                          username: myBooksCreds.username,
                          path: MYBOOKS_WEBDAV_ROOT_PATH,
                          server: myBooksCreds.serverUrl,
                        },
                      )
                    : _('Sign in to MyReader first to use it as your sync storage.')}
                </p>
                {myBooksCreds && (
                  <p className='px-1 text-sm'>
                    {isConnecting ? (
                      ...
                      <span className='text-error'>
                        {_('Connection failed')}: {connectionError}
                      </span>
                    ) : null}
                  </p>
                )}
                <div className='flex justify-end gap-2 pt-1'>
                  {myBooksCreds && connectionStatus !== 'success' && (
                    <button
                      type='button'
                  ...
                  <button
                    type='submit'
                    disabled={isConnecting || !myBooksCreds || connectionStatus !== 'success'}
                    className={clsx(
                      'btn btn-primary',
                      'h-10 min-h-10 rounded-lg border-0 px-5 text-sm font-medium',
                      'focus-visible:ring-primary/40 focus-visible:outline-none focus-visible:ring-2',
                      (isConnecting || connectionStatus !== 'success') && 'opacity-60',
                    )}
                  >
                    {_('Connect')}
                  </button>
